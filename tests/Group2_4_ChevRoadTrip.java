...
public class Group2_4_ChevRoadTrip extends BCATestScenario {
    public int runTest(){
        ...
        list.add(50.0); //{0.0, 100.0, 50.0}
        assertEquals(c1.roadTrip(list), 3, 0.1, "Days driven should be 3");
        assertEquals(c1.getMileage(), 250, 0.1, "Mileage should be 250");
        
        c1.recharge();
        list.add(101.0); //{0.0, 100.0, 50.0, 101.0}
        assertEquals(c1.roadTrip(list), 3, 0.1, "Days driven should be 3");

        c1.recharge();
        list.add(5.0); //{0.0, 100.0, 50.0, 101.0, 5.0}
        assertEquals(c1.roadTrip(list), 3, 0.1, "Days driven should be 3");

        ArrayList<Double> negativeList = new ArrayList<>();
        negativeList.add(-10.0);
        assertThrows(IllegalArgumentException.class, () -> {c1.roadTrip(negativeList);}, "Cannot drive negative miles");
